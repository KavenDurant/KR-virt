...
// 硬件信息相关状态 - 磁盘设备
const [nodeDisksData, setNodeDisksData] = useState<NodeDisksResponse | null>(null);
...
```typescript
// 获取节点PCI设备信息基础函数
const fetchNodePCIDataBase = useCallback(async (hostname: string) => {
  // 完整的错误处理和日志记录
...
// 获取节点磁盘设备信息基础函数
const fetchNodeDisksDataBase = useCallback(async (hostname: string) => {
  // 完整的错误处理和日志记录
...
useEffect(() => {
  if (sidebarSelectedHost) {
    ...
    // 自动加载硬件信息
    console.log(`🔧 [Hardware Info] 自动加载主机 ${sidebarSelectedHost.name} 的硬件信息`);
