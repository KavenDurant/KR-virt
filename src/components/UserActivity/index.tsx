...
export const UserActivityMonitor: React.FC<UserActivityMonitorProps> = ({
  ...
  const {
    ...
    onActivity: (event) => {
      if (finalConfig.debug && Math.random() < 0.1) { // 只记录10%的活动事件，避免日志过多
  ...
  }, [finalConfig.debug, isIdle, isPrompted, remainingTime, lastActiveTime, isLeaderTab, getDetailedState, finalConfig]);
