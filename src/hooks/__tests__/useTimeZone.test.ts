...
describe('useTimeZone Hook', () => {
  ...
  describe('useTimeZone', () => {
    it('应该正确转换ISO 8601时间字符串到本地时间', () => {
      ...
      expect(result.current.localTime).toMatch(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
    ...
    it('应该使用自定义格式', () => {
      ...
      expect(result.current.isValid).toBe(true);
      expect(result.current.localTime).toMatch(/\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}/);
    });
  ...
  describe('useTimeZoneBatch', () => {
    it('应该正确处理多个时间字符串', () => {
      ...
      result.current.forEach((timeResult) => {
        ...
        expect(timeResult.localTime).toMatch(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
  ...
  describe('useTimezoneInfo', () => {
    it('应该返回当前用户的时区信息', () => {
      ...
      expect(result.current.localTime).toMatch(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
      expect(result.current.utcTime).toMatch(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
    });
  });
