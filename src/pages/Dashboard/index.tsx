...
import { Row, Col, Card, Statistic, Progress, List, Typography, Table, Spin, Alert, Space, Tag, Tooltip } from 'antd';
import { 
  DashboardOutlined, 
  CloudServerOutlined, 
  HddOutlined, 
...
const mockResourceData = {
  ...
  systemHealth: {
    ...
    components: [
      ...
      { name: '存储资源', status: 'warning', message: '存储池 pool-02 使用率超过75%' },
      { name: '网络资源', status: 'normal', message: '网络连接正常' },
      { name: '系统服务', status: 'normal', message: '所有服务运行正常' }
    ]
  },
  performance: [
    { name: 'CPU使用率', current: 68, trend: 'up', history: [45, 52, 49, 55, 59, 66, 68] },
    { name: '内存使用率', current: 72, trend: 'stable', history: [70, 68, 65, 69, 71, 72, 72] },
    { name: '存储IO', current: 450, unit: 'IOPS', trend: 'down', history: [520, 490, 510, 480, 465, 455, 450] },
    { name: '网络吞吐量', current: 120, unit: 'Mbps', trend: 'up', history: [85, 90, 95, 105, 110, 115, 120] },
  ],
  alerts: [
    { id: 1, level: 'error', message: '物理机 host-03 离线', time: '10分钟前' },
    { id: 2, level: 'warning', message: '存储池 pool-02 使用率超过75%', time: '30分钟前' },
    { id: 3, level: 'warning', message: '虚拟机 vm-12 内存使用率高', time: '1小时前' },
    { id: 4, level: 'info', message: '系统自动备份完成', time: '3小时前' }
  ],
  recentEvents: [
    { id: 1, operation: '创建虚拟机', target: 'vm-48', user: 'admin', time: '2小时前' },
    { id: 2, operation: '修改网络配置', target: 'network-06', user: 'operator', time: '5小时前' },
    { id: 3, operation: '存储迁移', target: 'volume-23', user: 'admin', time: '1天前' },
    { id: 4, operation: '系统升级', target: '全局', user: 'admin', time: '2天前' }
  ]
};
  ...
  { title: '状态', dataIndex: 'status', key: 'status', 
    render: (status: string) => {
      const statusMap: Record<string, React.ReactNode> = {
        running: <Text type="success">运行中 <CheckCircleOutlined /></Text>,
        stopped: <Text type="secondary">已停止</Text>,
        error: <Text type="danger">错误 <WarningOutlined /></Text>
      };
      return statusMap[status] || status;
    } 
  },
  { title: 'CPU使用率', dataIndex: 'cpu', key: 'cpu',
    render: (cpu: number) => <Progress percent={cpu} size="small" strokeColor={cpu > 80 ? '#ff4d4f' : '#1890ff'} />
  },
  { title: '内存使用率', dataIndex: 'memory', key: 'memory',
    render: (memory: number) => <Progress percent={memory} size="small" strokeColor={memory > 80 ? '#ff4d4f' : '#1890ff'} />
  },
];
...
const Dashboard: React.FC = () => {
  ...
  const getHealthStyle = (status: string) => {
    switch(status) {
      case 'critical': 
        return { color: '#ff4d4f', bgColor: '#fff1f0', icon: <ExclamationCircleOutlined /> };
      case 'warning':
        return { color: '#faad14', bgColor: '#fffbe6', icon: <WarningOutlined /> };
      default:
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <Title level={2}><DashboardOutlined /> 系统仪表盘</Title>
          ...
          <Card className="health-card">
            <Title level={4}>
              ...
              {data.systemHealth.overall !== 'normal' && (
                <Tag color={data.systemHealth.overall === 'critical' ? 'error' : 'warning'} 
                  style={{ marginLeft: 8 }}>
                  {data.systemHealth.overall === 'critical' ? '严重警告' : '注意'}
                </Tag>
              )}
            </Title>
                ...
                return (
                  <Col xs={24} sm={12} md={6} key={index}>
                    ...
                    >
                      ...
                      <div className="health-component-message">{comp.message}</div>
                    </Card>
                  </Col>
                );
              })}
      ...
      {/* 资源概览部分 */}
      <Title level={4} className="section-title">资源概览</Title>
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} sm={12} md={12} lg={8} xl={6}>
          <Card className="resource-card">
            ...
            />
            <div className="resource-details">
              <Text>公网IP: {data.network.publicIPs} (已用 {data.network.usedIPs})</Text>
            </div>
          </Card>
        </Col>
      ...
      {/* 系统性能指标 */}
      <Title level={4} className="section-title">系统性能</Title>
      <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6} key={index}>
            <Card className="performance-card">
              <div className="performance-header">
                ...
                <Tooltip title={`趋势: ${item.trend === 'up' ? '上升' : item.trend === 'down' ? '下降' : '稳定'}`}>
                  <span className="performance-trend" style={{ color: getTrendIcon(item.trend).color }}>
              ...
              <Statistic 
                value={item.current}
                suffix={item.unit}
                valueStyle={{ color: item.trend === 'up' ? '#ff4d4f' : item.trend === 'down' ? '#52c41a' : '#1890ff' }}
              />
              <div className="performance-chart">
                {item.history.map((val, i) => (
                  <div 
                    ...
                    style={{ 
                      height: `${val/6}%`,
                      backgroundColor: item.trend === 'up' ? '#ff4d4f' : item.trend === 'down' ? '#52c41a' : '#1890ff',
                      opacity: 0.3 + (i * 0.1)
                    }}
                  />
                ))}
