...
export class SecurityUtils {
  ...
  // 验证文件签名
  static async validateFileSignature(file: File, expectedHash?: string): Promise<boolean> {
    try {
  ...
  // 检查文件内容安全性
  static validateFileContent(content: string): { isValid: boolean; risks: string[] } {
    const risks: string[] = [];
    ...
    // 检查SQL注入模式
    if (/(\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER)\b)/gi.test(content)) {
