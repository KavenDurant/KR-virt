...
describe('集群解散流程测试', () => {
  ...
  it('解散集群成功后应该退出登录并跳转到bootstrap页面', async () => {
    ...
    vi.spyOn(clusterInitService, 'dissolveCluster').mockResolvedValue(mockDissolveResult);
  ...
  it('解散集群失败时不应该执行登出和跳转', async () => {
    ...
    vi.spyOn(clusterInitService, 'dissolveCluster').mockResolvedValue(mockDissolveResult);
    vi.spyOn(loginService, 'logout').mockResolvedValue({ success: true, message: '登出成功' });
  ...
  it('解散集群异常时不应该执行登出和跳转', async () => {
    // Mock 解散集群API抛出异常
    vi.spyOn(clusterInitService, 'dissolveCluster').mockRejectedValue(new Error('网络错误'));
    vi.spyOn(loginService, 'logout').mockResolvedValue({ success: true, message: '登出成功' });
...
describe('AppBootstrap状态检查流程', () => {
  it('应该调用status接口检查集群状态', async () => {
    ...
    vi.spyOn(clusterInitService, 'checkClusterStatus').mockResolvedValue(mockStatusResult);
